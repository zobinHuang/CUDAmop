/*!
 * \file    aligned.cu
 * \brief   Aligned memory accessing version of matrix multiplication
...
int main(){
  ...
  // transpose matrix B
  transposeSquareMatrix(matrix_B);
  ...
  alignedSquareMatrixMul<<<blocks, threads>>>(d_matrix_A, d_matrix_B, d_matrix_C, N);
  PROFILE(
  ...
  // transpose matrix B back to normal alignment
  transposeSquareMatrix(matrix_B);
