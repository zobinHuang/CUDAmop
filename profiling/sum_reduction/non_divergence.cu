...
int main(){
  ...
  nonDivergenceSumReduction<<<NUM_BLOCKS, NUM_THREADS_PER_BLOCK, shared_memory_size>>>(d_source_array, d_destination_array);
  PROFILE(
  ...
  nonDivergenceSumReduction<<<1, NUM_THREADS_PER_BLOCK>>>(d_destination_array, d_destination_array);
  PROFILE(
    cudaDeviceSynchronize();
    nvtxRangePop();
  )
